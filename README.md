...
# Feedback
Please leave an issue or email me if you find any problems with the app! It is fully functional and we've run complete games with custom-written AIs, but there still may be underlying bugs.
...
## CHANGELOG
1.0
- More renderer improvements (thanks jhoak!)
- Minimap shows camera and can be interacted with
- Python now exports zipped states (so games are way smaller)
- Fix visual bug where constructed buildings aren't cleaned up
