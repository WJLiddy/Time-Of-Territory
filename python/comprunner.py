...
def runais(world_state,players,player_discovery,tick,sname):
    ...
    for team in range(PLAYER_COUNT):
        ...
        # delete outfiles, since we read it already. Unity only needs states + transitions.
        if exists(out_file_name(sname,tick,team)):
            remove(out_file_name(sname,tick,team))
...
def play():
    ...
    while(True):
        ...
        # zip and remove the raw state.
        with ZipFile(os.path.splitext(state_filename(sname,tick))[0]+".zip",'w',compression=ZIP_DEFLATED,compresslevel=9) as zip:
            zip.write(state_filename(sname,tick),arcname="state.json")
        os.remove(state_filename(sname,tick))
        ...
        # zip and remove the raw moves.
        with ZipFile(os.path.splitext(sname + str(tick) + "move.json")[0]+".zip",'w',compression=ZIP_DEFLATED,compresslevel=9) as zip:
            zip.write(sname + str(tick) + "move.json",arcname="move.json")
        os.remove(sname + str(tick) + "move.json")
