...
def infantry_rush(world_state,peasant, enemy_exists):

    # attack anyone we see.
    cdir = None
    if(enemy_exists):
        cdir = path_to([peasant["x"],peasant["y"]],world_state,lambda x: x is not None and x != "u" and x["team"] != peasant["team"] and x["team"] != -1)
    if(cdir == None):
        ...
        for v in range(4):
            ...
            if(world_state[scout_dest[0]][scout_dest[1]] == "u" and cdir == None):
                cdir = path_to_coord([peasant["x"],peasant["y"]],world_state,scout_dest)
        if(cdir == None): # no path to anything, return.
            return {}

    # see if we can attack anything.
    if(world_state[cdir[0]][cdir[1]] is None):
        return {"id":peasant["id"],"command":"m","arg":[cdir[0] - peasant["x"] ,cdir[1] - peasant["y"]]}
    elif(world_state[cdir[0]][cdir[1]]["type"] is not None):
        return {"id":peasant["id"],"command":"k","arg":world_state[cdir[0]][cdir[1]]["id"]}
    return {}
...
def run(world_state,players,team_idx):
    ...
    iz = get_tiles(world_state,lambda x: x["team"] == team_idx and x["type"] == "i")
    hs = get_tiles(world_state,lambda x: x["team"] == team_idx and x["type"] == "h")
    bs = get_tiles(world_state,lambda x: x["team"] == team_idx and x["type"] == "b")
    ...
    if(len(vs) == 1):
        ...
        if(len(bs) == 0):
            ...
            if([v0["x"],v0["y"]] != barbldpair(team_idx, len(world_state))[1]):
                ...
                return [{"id":v0["id"],"command":"b","arg":barbldpair(team_idx,len(world_state))[0]}]
    ...
    for i in iz:
        commands.append(infantry_rush(world_state,i, len(es) > 0))
